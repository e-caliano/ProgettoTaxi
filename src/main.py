...
def luogo_viaggio_maggiore(anno,mesi,percorso_directory):
    """
    function per trovare il borough dove si è svolto il viaggio con durata più elevata
    :param anno: str
    :param mesi: list
    :param percorso_directory: str
    :return: list
    """
...
def luogo_viaggio_minore(anno,mesi,percorso_directory):
    """
    function per trovare il borough dove si è svolto il viaggio con durata più breve
    :param anno: str
    :param mesi: list
    :param percorso_directory: str
    :return: list
...
def dizionario_per_plot(mesi):
    """
    function per la creazione di un dict per assocciare ad ogni mese (chiave) le durate massime e minime corrispondenti (valori)
    :param mesi:lista
    :return: dict
    """
    min_durata_corsa= elaboration.viaggio_più_breve(dfs)[1]
    max_durata_corsa=elaboration.viaggio_più_lungo(dfs)[1]
...
def crea_istogramma_durata_viaggi(dizionario):
    """
    Function per la creazione di un istogramma della durata dei viaggi, suddivisi per mese.
...
print(elaboration.durata(df)) # per vedere le durate di tutti i borough in dataframe
